...
class Transformation_Grid:
  def __init__(self, grid):
    ...
    self.dist_reverse = self.indices_reverse.copy()[self.dist_from_axis + 1:]
  ...
  def shift45deg_cw(self, grid, quadrant):
    ...
    if quadrant == 1:
      dist_reverse_last = self.indices_reverse.copy()[: self.dist_from_axis]
      ...
      for i in range(self.dist_from_axis):
        ...
        if grid[self.dist_from_axis][j] == 1:
          ...
          temp_grid = self.shift_down(temp_grid, self.dist_from_axis, j, k)
        
        if grid[self.dist_from_axis - k][j] == 1:
          temp_grid[self.dist_from_axis - k][j] = 1
          temp_grid = self.shift_down(temp_grid, self.dist_from_axis - k, j, k)                
    ...
    if quadrant == 2:
      for i in range(self.dist_from_axis):
        ...
        if grid[j][self.dist_from_axis] == 1:
          ...
          temp_grid = self.shift_right(temp_grid, j, self.dist_from_axis, k)
          
        # Diagonal elements
        if grid[j][self.dist_from_axis - k] == 1:
          temp_grid[j][self.dist_from_axis - k] = 1
          temp_grid = self.shift_right(temp_grid, j, self.dist_from_axis - k, k)
    ...
    if quadrant == 3:      
      for i in range(self.dist_from_axis):
        ...
        if grid[self.dist_from_axis][j] == 1:
          temp_grid[self.dist_from_axis][j] = 1
          temp_grid = self.shift_up(temp_grid, self.dist_from_axis, j, k)

        # Diagonal elements
        if grid[self.dist_from_axis + k][j] == 1:
          temp_grid[self.dist_from_axis + k][j] = 1
          temp_grid = self.shift_up(temp_grid, self.dist_from_axis + k, j, k)      
    ...
    if quadrant == 4:
      ...
      for i in range(self.dist_from_axis):
        ...
        if grid[l][self.dist_from_axis] == 1:
          ...
          temp_grid = self.shift_left(temp_grid, l, self.dist_from_axis, k)
          
        if grid[l][self.dist_from_axis + k] == 1:
          temp_grid[l][self.dist_from_axis + k] = 1
          temp_grid = self.shift_left(temp_grid, l, self.dist_from_axis + k, k)

    return temp_grid


  # Counter-clockwise 
  def shift45deg_ccw(self, grid, quadrant):
    temp_grid = []
    temp_grid.extend(range(len(grid)))
    ...
    if quadrant == 2:
      for i in range(self.dist_from_axis):
        ...
        if grid[j][self.dist_from_axis] == 1:
          temp_grid[j][self.dist_from_axis] = 1
          temp_grid = self.shift_left(temp_grid, j, self.dist_from_axis, k)
          
        # Diagonal elements
        if grid[j][self.dist_from_axis + k] == 1:
          temp_grid[j][self.dist_from_axis + k] = 1
          temp_grid = self.shift_left(temp_grid, j, self.dist_from_axis + k, k)
    ...
    if quadrant == 4:
      ...
      for i in range(self.dist_from_axis):
        ...
        if grid[l][self.dist_from_axis] == 1:
          temp_grid[l][self.dist_from_axis] = 1
          temp_grid = self.shift_right(temp_grid, l, self.dist_from_axis, k)
          
        if grid[l][self.dist_from_axis - k] == 1:
          temp_grid[l][self.dist_from_axis - k] = 1
          temp_grid = self.shift_right(temp_grid, l, self.dist_from_axis - k, k)
  ...
  def rotate_right(self):
    ...
    temp_grid[self.width - 1 - self.dist_from_axis][self.width - 1 - self.dist_from_axis] = self.grid[self.width - 1 - self.dist_from_axis][self.width - 1 - self.dist_from_axis]
  ...
  def rotate_left(self):
    ...
    grid1 = self.shift45deg_ccw(grid, 1)      
    grid2 = self.shift45deg_ccw(grid, 2)
    grid3 = self.shift45deg_ccw(grid, 3)
    grid4 = self.shift45deg_ccw(grid, 4)
